...
class HierarchicalBatchTopKSAE(nn.Module):
    ...
    def encode(self, x: t.Tensor, return_active: bool = False, use_threshold: bool = False):
        ...
        level_latents_sparse, post_relu_acts_last_layer = [], None
        for i in range(self.n_levels):
            ...
            if use_threshold:
                if i == 0:
                    sparse_acts = post_relu_acts * (post_relu_acts > self.thresholds[i][0])
                else:
                    ...
                    thresholds_for_broadcast = self.thresholds[i].view(1, num_groups, 1)
                    sparse_acts = grouped_acts * (grouped_acts > thresholds_for_broadcast)
            else:
                if i == 0:
        ...
        for i in range(self.n_levels - 2, -1, -1):
            latent_to_multiply = level_latents_sparse[i]

            # 상위 레벨 latent를 올바르게 reshape
            # 예: level0 -> (B,10) -> (B,10,1,1), level1 -> (B,40) -> (B,10,4,1)
            target_shape = [batch_size] + list(self.latent_sizes_list[:i+1]) + [1] * (len(self.latent_sizes_list) - (i+1))
            latent_to_multiply_reshaped = latent_to_multiply.view(*target_shape)

            final_latent = final_latent * latent_to_multiply_reshaped
...
class HierarchicalBatchTopKTrainer:
    def __init__(
        ...
        device: Optional[str] = None, wandb_name: str = "HierarchicalBatchTopKSAE",
