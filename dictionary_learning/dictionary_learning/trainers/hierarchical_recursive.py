...
class HierarchicalSAE_Recursive(nn.Module):
    def __init__(self, activation_dim: int, dict_size: int, k: int, lower_level_latent_sizes: List[int], lower_level_ks: List[int]):
        ...
        for i in range(self.n_levels):
            # 각 레벨의 출력 차원은 기존과 동일 (누적 곱)
            output_dim = int(np.prod(self.latent_sizes_list[:i+1]))
            
            # 인코더 추가
            self.encoders.append(nn.Linear(current_input_dim, output_dim, bias=True))
            
            # 다음 인코더의 입력 차원은 현재 인코더의 출력 차원이 됨
    ...
    def encode(self, x: t.Tensor, return_active: bool = False, use_threshold: bool = False):
        ...
        ## 변경/추가된 부분: 인코더를 연쇄적으로 통과시키며 희소 잠재 벡터 생성
        ...
        for i in range(self.n_levels):
            ...
            # 다음 인코더의 입력으로 사용하기 위해 저장
            current_latent = post_relu_acts
...
class HierarchicalSAERecursiveTrainer:
    def __init__(
