...
class HBatchTopKSAE(Dictionary, nn.Module):
    def __init__(self, activation_dim: int, dict_size: int, k1: int, k2: int):
        super().__init__()
        self.activation_dim = activation_dim
        self.dict_size = dict_size

        assert isinstance(k1, int) and k1 > 0, f"k1={k1} must be a positive integer"
        assert isinstance(k2, int) and k2 > 0, f"k2={k2} must be a positive integer"
        self.register_buffer("k1", t.tensor(k1, dtype=t.int))
        self.register_buffer("k2", t.tensor(k2, dtype=t.int))
        self.register_buffer("threshold1", t.tensor(-1.0, dtype=t.float32))
        self.register_buffer("threshold2", t.tensor(-1.0, dtype=t.float32))
